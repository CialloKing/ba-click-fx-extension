...
- Visual preferences—including enabled states, color, opacity, scale, render mode, maximum device-pixel ratio, effect-parameter overrides, language, and motion choices—are saved with the browser-provided `storage.sync` API.
...
BA Click FX 只为实现可见的点击特效、光标拖尾与站点开关而在本地处理鼠标坐标和当前网页 origin。鼠标坐标只存在于内存中，不会记录或保留。颜色、不透明度、缩放、渲染模式、最大设备像素比、圆环/碎片/Bloom/Hit/Flare/拖尾参数、开关、语言和动态偏好通过浏览器提供的 `storage.sync` API 保存；从 1.0.5 起，用户主动禁用的网站 origin 通过 `storage.local` 保存在当前浏览器配置中。
