...
| Enabled states, color, opacity, scale, quality, render mode, maximum DPR, output/isolated compositing, light-background contrast, click/trail timing, preset, versioned effect-parameter overrides, language, motion | Popup/options/shared settings | Browser `storage.sync` until changed/reset/uninstalled | Browser-vendor sync only when enabled | Apply the user's visual, performance, interface, and accessibility preferences |
